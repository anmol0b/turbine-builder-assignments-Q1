...
impl<'info> Deposit<'info> {
    pub fn deposit(
        ...
        // Check user has sufficient balance for initial deposit
        if self.mint_lp.supply == 0 {
            require!(
                self.user_x.amount >= max_x && self.user_y.amount >= max_y,
        ...
        {
            // Initial deposit - use max amounts
            (max_x, max_y)
        } else {
        ...
        // Check user has sufficient balance for calculated amounts
        require!(
            self.user_x.amount >= x && self.user_y.amount >= y,
            AmmError::InsufficientBalance
        );
