...
impl<'info> ThawNft<'info> {
    pub fn thaw_nft(&mut self) -> Result<()> {
        self.validate_thaw_eligibility()?;
        self.execute_thaw()?;
        
        Ok(())
    }

    fn validate_thaw_eligibility(&self) -> Result<()> {
        Ok(())
    }

    fn execute_thaw(&self) -> Result<()> {
        ...
        UpdatePluginV1CpiBuilder::new(&self.core_program.to_account_info())
            ...
            .map_err(|_| error!(MPLXCoreError::ThawFailed))?;

        msg!("NFT thawed: {}", self.asset.key());
